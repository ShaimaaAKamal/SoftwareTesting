package Tests.Auth.AccountFunctions;

import Pages.Auth.AccountFunctions.Downloads_Page;
import Pages.Auth.AccountFunctions.Subscriptions_Page;
import Pages.Auth.Account_Page;
import Pages.Auth.Login_Page;
import Pages.Home_Page;
import Tests.BaseTest;
import org.testng.annotations.AfterMethod;
import org.testng.annotations.BeforeMethod;
import org.testng.annotations.Test;

public class Subscriptions extends BaseTest {
    Subscriptions_Page subscriptionsPage;
    Home_Page home;
    Account_Page accountPage;
    Login_Page login;
    @BeforeMethod
    public void preCondition() {
        driver.get("http://localhost:8888/opencart/");
        subscriptionsPage = new Subscriptions_Page(driver);
        accountPage=new Account_Page(driver);
        home=new Home_Page(driver);
        login=new Login_Page(driver);
        home.clickMyAccount();
        home.clickLogin();
        login.enterEmail("tete@gmail.com");
        login.enterPassword("Test710@");
        login.submitForm();
        waitForVisible(accountPage.editInformation);
        accountPage.navigateToSubscriptionsUsingSidebar();
        waitForVisible(subscriptionsPage.pageTitle);
    }

    @AfterMethod
    public void after(){
        accountPage.logout();
    }

    @Test(priority = 0)
    public void assertInpage() {
        subscriptionsPage.assertInOrderHistoryPage();
    }

    @Test(priority = 1)
    public void clickContinue() {
        subscriptionsPage.clickContinue();
        waitForVisible(accountPage.editInformation);
    }
}
